...
def _generate_input(M: int, K: int, N: int) -> tuple[Any, ...]:
    ...
    scale_a = torch.randn(num_group_m, num_group_k, dtype=torch.float32, device=device)
    scale_b = torch.randn(num_group_k, num_group_n, dtype=torch.float32, device=device)
