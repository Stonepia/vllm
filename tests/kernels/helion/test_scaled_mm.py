...
def _generate_input(M: int, K: int, N: int) -> tuple[Any, ...]:
    ...
    a = torch.randn(M, K, dtype=torch.float32, device=device).to(in_dtype)
    b = torch.randn(N, K, dtype=torch.float32, device=device).to(in_dtype)
    b = b.t()
    scale_a = torch.randn(M, 1, dtype=torch.float32, device=device)
    scale_b = torch.randn(N, 1, dtype=torch.float32, device=device)
...
def _reference_scaled_mm(
    ...
    """Device-agnostic reference for scaled_mm.

    baseline() in vllm/kernels/helion/ops/scaled_mm.py calls
    ops.cutlass_scaled_mm, which is CUDA-only (CUTLASS). On platforms
    without CUTLASS (e.g. XPU), use torch._scaled_mm -- the native XPU op
    for fp8 scaled matmul -- instead, treating it as this platform's
    torch.ops._C-equivalent, per project decision.

    torch._scaled_mm has two hardware limitations CUTLASS doesn't share,
    confirmed on this platform: it only supports fp8 inputs (raises
    "Invalid scaling configuration" for int8), and it requires the mat2
    dimensions to be divisible by 16 (raises for e.g. the (512, 20486,
    1024) MNK_FACTORS case). The plain-math float32 fallback below covers
    both cases; it is a valid ground truth regardless of dtype or shape.
    """
    ...
    out = scale_a.to(torch.float32) * out
    out = scale_b.to(torch.float32).T * out
...
class TestScaledMmCorrectness:
    ...
    def test_scaled_mm(
        ...
        if is_floating_point_type(in_dtype):
            a = (0.25 * torch.rand((M, K), dtype=torch.float32, device=device)).to(
                in_dtype
            )
            b = (0.25 * torch.rand((N, K), dtype=torch.float32, device=device)).to(
                in_dtype
            )
        else:
            a = torch.randint(-32, 32, (M, K), dtype=in_dtype, device=device)
            b = torch.randint(-32, 32, (N, K), dtype=in_dtype, device=device)
        ...
        if current_platform.is_cuda():
            ...
            # baseline() calls CUTLASS (CUDA-only); use the portable
            # float32 math reference on other platforms (e.g. XPU).
            c_actual = _reference_scaled_mm(a, b, scale_a, scale_b, out_dtype, bias)
