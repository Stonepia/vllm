...
def generate_inputs() -> dict[CaseKey, tuple[Any, ...]]:
    ...
    for M, (K, N) in product(m_size_list, b_shape_list):
        ...
        a = (scale * (0.5 + torch.rand(M, K, dtype=torch.float32, device=device))).to(
            in_dtype
        )
        b = (scale * (0.5 + torch.rand(N, K, dtype=torch.float32, device=device))).to(
            in_dtype
        )
        b = b.t()
        scale_a = 0.5 + torch.rand((M, 1), dtype=scale_dtype, device=device)
        scale_b = 0.5 + torch.rand((N, 1), dtype=scale_dtype, device=device)
        bias = 0.5 * (torch.rand(N, dtype=out_dtype, device=device) - 0.5)
