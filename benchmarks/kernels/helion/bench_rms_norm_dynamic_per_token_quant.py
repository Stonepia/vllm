...
# One [num_tokens, hidden_size] shape per Qwen3 model's hidden_size, at a
...
def main() -> None:
    ...
    for name, hidden_size in HIDDEN_SIZES.items():
        ...
        compiled_ms = bench(lambda buf=compiled_buf: torch_compile_baseline(*buf))
        kern_ms = bench(lambda buf=kernel_buf: kernel(*buf))
        ...
        if torch_ops_c_available:
            ...
            ops_c_ms = bench(lambda buf=opsc_buf: torch_ops_c_baseline(*buf))
